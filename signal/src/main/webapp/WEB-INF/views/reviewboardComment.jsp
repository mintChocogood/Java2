					...
					$('.pageNum')
						.on(
							...
							function (e) {
								...
								location = "/board/review/get?currPage=${param.currPage}&postNo=${__REVIEW__.postNo}&commentCurrPage="
									+ selectedPageNum;
							});
				});
				...
				$(function () {
					$('.replyWriteBtn')
						.on(
							'click',
							function () {
								location = "/board/review/get?currPage=${param.currPage}&postNo=${__REVIEW__.postNo}&commentCurrPage=" + 1;
							});
				});
				$(function () {
		...
		<body>
					...
					<div class="reply_list">
						...
						<div class="edit-area" style="display: none;">
							...
							<button type="button" class="submit-edit-btn" data-comment-no="${comment.commentNo}"
								data-post-no="${__RIVIEW__.postNo}" data-content="${comment.content}">수정 제출</button>
						</div>
					</div>
				</c:forEach>
			</div>
			...
			<div class="board_page">
				<c:if test="${__commentPage__.commentPrev}">
					<div class="Prev">
						<a
							href="/board/review/get?currPage=${param.currPage}&postNo=${__REVIEW__.postNo}&commentPage=${__commentPage__.startCommentPage - 1}">Prev</a>
					</div>
				</c:if>
				...
				<c:if test="${__commentPage__.commentNext}">
					<div class="Next">
						<a
							href="/board/review/get?currPage=${param.currPage}&postNo=${__REVIEW__.postNo}&commentPage=${__commentPage__.endCommentPage + 1}">Next</a>
					</div>
				</c:if>
			</div>
			...
			<!-- 댓글 작성 -->
			<form action="/board/review/reviewReply" method="POST">
